...
The agent-agnostic core of the issue workflow. It knows nothing about Claude Code, which is why it lives at the plugin's top level instead of under `hooks/` — the hooks call it, and `~/.claude/workflow` is its stable address for anything else that does.
...
| `templates/issue-forms/` | The markdown GitHub issue templates (bug · enhancement · idea · dump) installed into a repo's `.github/ISSUE_TEMPLATE/`. Each pre-fills the issue anatomy (`## Description` then `## Plan`) and auto-applies `status:inbox` + its `type:` label |
...
Never-asked and declined are personal, not project facts (Ian 2026-07-24): a teammate seeing `enabled: false` in a shared repo would read it as the project declining when it was one developer undecided. Only a real yes belongs in the repo.
...
```sh
bash ~/.claude/workflow/standards.sh --enable [repo]    # write the committed opt-in, then heal
bash ~/.claude/workflow/standards.sh --decline [repo]   # record it in the USER file; never offered again
bash ~/.claude/workflow/standards.sh --state [repo]     # enabled | disabled | declined | undecided | nogit
bash ~/.claude/workflow/standards.sh --announce [repo]  # the offer line, for a hook to relay
```
...
`setup/modules/09-claude.sh` symlinks this folder to `~/.claude/workflow`, so every reference in the harness — the `workflow/standards` hook, the spec, the skills — resolves through that path. The hook takes a `WORKFLOW_DIR` override; the tests use it to point at this folder directly.

Run it by hand against any repo:

```sh
bash ~/.claude/workflow/standards.sh [repo-root]
```
...
This folder is the seed of a future installable kit — the workflow defined once, installed per agent and per developer, in repos beyond Ian's own. Tracking: issue #2. Keep it self-contained: nothing here may depend on `~/.claude`, on Claude Code, or on anything else in this repo.
