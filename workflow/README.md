...
| `publish.sh` | Refreshes the roster on the home repo's default branch, then builds the tower project in `~/.workkit/tower` and pushes the output to the home repo's `gh-pages` branch — the build and the push only when `site.publish` says so, and a switch turned off takes the published site down |
...
**The roster is not part of the site** (issue #111). `data/repos.json` on the home repo's default branch feeds the CLOUD BRIEF as well as the published dashboard, so it is composed and pushed ABOVE the publish switch and above every build check — it needs node, git and the clone, and nothing that publishing needs. A machine with the switch off, or without the tooling to build, still leaves the brief a current list. Whatever else the day changed in the clone rides in the same commit, one `chore(home): refresh the repo list`; a push that does not land is warned about and carries out as the exit code, and never costs the site a publish it can still make.
