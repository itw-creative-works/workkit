...
| `publish.sh` | Builds the tower project in `~/.workkit/tower` and pushes the output to the home repo's `gh-pages` branch — only when `site.publish` says so, and with the board snapshot baked in only when `site.board` does |
