...
//
// CLI:
//   node changelog.js <file> [--added-only] [--staged]
 ...
 * @returns {Array<{line: number, rule: string, message: string}>}
 */
const lintText = (text, onlyLines = null) => parseEntries(text)
...
const main = (argv) => {
  ...
  if (!file) {
    console.error('usage: changelog.js <file> [--added-only] [--staged]');
    return 2;
  }
  ...
  const violations = lintText(text, onlyLines);
