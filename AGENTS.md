...
**How the day is handed over** (issue #82). The scheduled morning's first move is `gh workflow run brief.yml` on this repo, made only once `gh secret list` names `CLAUDE_CODE_OAUTH_TOKEN` there — the workflow accepts a dispatch the moment its file exists, secrets or not, and a runner that cannot compose is a silently briefless morning. A dispatch that lands is the whole brief; nothing is composed or published here, only logged, and the site publish still follows. `.github/workflows/brief.yml` runs `jobs/claude-cloud.sh` on a runner (dispatch plus a 17:30 UTC cron backup, one concurrency group), which REFUSES to run anywhere but Actions — it rewrites `~/.workkit`, which on a laptop is the real roster — then seeds a synthetic one from `WORKKIT_HOME_SLUG`, builds its repo list from the slugs this machine already published to the home repo's `gh-pages` (`data/repos.json`) — falling back to the home repo alone, never an empty board — composes and sends the same payload, and posts the same Discussion through `jobs/brief-publish.sh`, the one function both runners source. Only the brief travels: the summaries and the site read a machine a runner is not. Three things differ from the laptop on purpose — failures are LOUD there, since the Actions log is the delivery; there is no notification; and the digest body never reaches that log, which gets the headline and a byte count as proof of life. A dispatch that cannot be made is a silent false and the full local brief runs as before; the check-before-post is what lets the dispatch, the cron and a local run overlap.
