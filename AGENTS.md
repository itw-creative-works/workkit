...
Eleven, namespaced `workkit:<name>`: `feature` · `interview` · `diagnose` · `review` · `simplify` · `triage` · `whats-next` · `state` · `migrate` · `parallel` · `ship`. One `SKILL.md` each, which is that skill's own home — what it does, when it fires, and how it runs.
