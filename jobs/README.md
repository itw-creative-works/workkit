...
| `../.github/workflows/brief.yml` | what runs `claude-cloud.sh` — `workflow_dispatch` for the laptop's trigger, a 17:30 UTC cron as the backup |
...
**Then it tries the cloud first** (issue #82). The scheduled brief — and only that; a `<message>` run and `--now` never do this — starts with `gh workflow run brief.yml` on this repo. **The secrets are checked before the day is handed over:** `gh secret list` on that same repo has to name `CLAUDE_CODE_OAUTH_TOKEN`, because a dispatch succeeds the moment the workflow file is on the default branch — secrets or not — and a runner without the token composes nothing, which would make every morning in that window silently briefless. A dispatch that lands is the whole brief: the log records `brief: dispatched brief.yml on <slug>`, nothing is composed or published here, and the morning's delivery is the Discussion the runner posts. A dispatch that cannot be made — no network, a `gh` that is not authenticated, a repo without the workflow, a repo whose secrets are not set yet — is a silent false and the full local brief runs exactly as it always did. Either way the site publish still runs afterwards: the site is this machine's to build.
