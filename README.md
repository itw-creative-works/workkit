...
| `safety/release-taken` | before the release commit, and before `npm publish` | Checks the version you are about to release against the places it could already exist: npm, for every package with publish intent (workspace members included), and the repo's GitHub releases for the `v<version>` tag. A version somebody already published bounces here, while the number is still free to change, instead of at the failed publish on top of a finished commit and tag. A check that cannot be made (offline, no auth) says so and stands aside |
...
| `safety/proof-guard` | before a `gh issue edit` that adds `status:complete`, or a `gh issue close` | Blocks the move when the issue carries no comment starting `Proof:`, the record of which test layers actually ran. Closing something that was never built (`--reason "not planned"`) passes, and a `gh` that cannot answer stands the check down out loud |
| `safety/suite-guard` | before a Bash command that runs the whole test suite | Bounces the full suite run by hand: the commit gate runs it at the commit, so running it yourself first pays the same minutes twice. Run the narrowest test that proves your change instead, and put `WORKKIT_SUITE=1` in front of the command when you do want the whole thing |
