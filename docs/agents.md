...
Test scope is doctrine for every class, the manager included (#152, #243): a worker's mid-work proof is the test files it touched, red-green on the new cases; a verifier runs the narrowest command that checks the claim. None of them runs a package or root suite unless the brief asks or the finding is suite-scoped. The commit gate owns suites, and `safety/suite-guard` bounces the hand-run from any class. The rule is the spec's: `project-state.md` § The proof.
