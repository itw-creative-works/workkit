...
- **The proof** (owner ruling, 2026-09-02, #217). Tests green means green at every layer the change has a surface on:
  ...
  - No `Proof:` line means not built. The SHIP is what names it (owner ruling, 2026-09-09, #219): it lists every item at `status:qa` or `status:complete` whose comments carry no such line and the owner calls each one, since a spoken pass reaches `complete` with nothing written down, and a ship is the moment a missing proof matters. Never a block, and never a `Proof:` line written on the owner's behalf.
