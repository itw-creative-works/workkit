...
| `status:building` | Authorized and in flight — the work has started. Carried through build, verify, and the wait to ship; the ship close ends it. |
...
Three surfaces, all converging on issues. There is no store outside a repo: the capture that belongs to no project lands in the tower clone's own inbox, which is a participating repo's inbox like any other.
...
3. **`.workkit/inbox.md`** — the local, gitignored free-form dump for offline moments. Triage drains it into issues the same way. The standards script creates it empty (with its capture header) so dumping is always zero-friction; it never overwrites one that has entries. It is the OWNER'S SCRATCHPAD: its contents are read during a triage run and at no other time — counting the entries and appending to it are always free, and the `safety/inbox-guard` hook blocks any other read, opened by the marker the `workkit:triage` skill records. The engine's capture CLI (`wk.sh note "the thought"`) appends here from any shell — and standing outside every participating repo, to the **tower clone's own** `~/.workkit/tower/.workkit/inbox.md`, drained by triage into whichever repo each entry belongs to; with no clone on this machine it writes nothing and names `workkit setup`.
...
Its one job is keeping the LOCAL AGENT on task across a compaction, a resume, or a restart, which is why it is READ BACK at every session start (the `docs:session` hook injects it on every source, silent when the file holds only its header). It is a queue, not a journal, and not long-term storage: past ~40 content lines the injection says so, because everything durable belonged on an issue the moment it existed. A session.md growing into the retired `PROGRESS.md` shape is the failure the bar exists to catch.
...
- **Working** = the issue is assigned and carries `status:building`. Your queue is `gh issue list --assignee @me`.
...
| Surface | Job |
|---|---|
| `settings.json` | This machine's own decisions: the `repos` map (the roster, below, plus the declines), the `"home": "<owner>/<repo>"` slug, and the cached GitHub node ids |
...
**2. The home repo** — one private GitHub repo, `<login>/workkit`, created by `workkit setup`, named in `~/.workkit/settings.json` as `"home": "<owner>/<repo>"`, and cloned at `~/.workkit/tower`. It is the queue for everything that belongs to no single repo, and it is a repo like any other — the standard's labels, its issue anatomy, its templates, and its own committed `.workkit/settings.json` saying yes.

It is also a real PROJECT: the clone is the tower's dashboard, seeded from the kit's own `tower/app` and shaped like any other site project — a brand root with `apps/`, `config/`, its own `.gitignore`. Its `config/workkit.json` carries `site.url` (the published dashboard's optional custom URL) and `site.board` (default `false`). Nothing generated is committed as source: there is no project list in it, and no build output on its default branch.

- **Setup creates it and nothing else ever does.** The wizard detects the login, creates the private repo, clones it into `~/.workkit/tower`, seeds the project when the clone is empty, installs its dependencies, and enables Discussions and Pages. Each step is idempotent, each failure warns with the exact fix and the wizard carries on. **Nothing is ever converted or adopted**: an absent path is cloned, the right clone is a no-op, and anything else already at that path stops the home steps. The daily path and the session hook never create, clone, or enable anything — the one exception is the `gh-pages` branch, which is generated output and belongs to the publish that generates it.
...
- Its **issues** are the cross-project and business queue: whatever triage cannot route to a project repo. Its `.workkit/inbox.md` is where a capture outside every participating repo lands; there is no user-level inbox.
...
**The published dashboard.** The tower project in `~/.workkit/tower`, built locally by the engine and pushed to the home repo's `gh-pages` branch, which GitHub Pages serves from the branch ROOT — so no build output ever exists on the default branch and no folder is named for a Pages rule. The board readable from a phone, with the sweep it shipped with baked in at `data/board.json` when `site.board` says so (Pages is public even on a private repo, so that switch is the owner's and defaults to off). The daily job publishes after the brief, and `workkit publish` does it on demand; a machine without the build tooling, or a clone that has diverged, skips cleanly and forces nothing. `config/workkit.json`'s `site.url` becomes the CNAME.
