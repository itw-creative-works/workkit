...
- The queue is a QUERY, never a file: `gh issue list`. Nothing about work state is stored in the repo. Every kit query reads up to 1,000 issues (`--limit 1000`, #214): a filter is applied before the cap, and `gh` never says when a cap cut the answer.
...
- **The proof** (owner ruling, 2026-09-02, #217). Tests green means green at every layer the change has a surface on:
  - unit: known inputs.
  - integration: the pieces together, real where they can run real.
  - end to end: the user's own path through the real surface. Its unit is a FLOW, not an issue: one path a person takes, happy or sad (owner ruling, 2026-09-03). A park names the flows that cover the change; a flow that already exists is named, not rewritten.
...
Before a deliberate compaction, the `workkit:checkpoint` skill APPLIES the conversation to the board: every owner verdict, decision and question enumerated first, then filed or commented onto its issue by the triage table, and every stage the owner's words moved flipped (a spoken "X is done" on a qa item IS the grant to `status:complete`; owner ruling, 2026-08-27), with `session.md` trimmed to what is in flight, so the chat's findings survive on the board instead of in the scrollback.
...
Wherever an issue is restated outside its own page (the status digest, the triage and checkpoint Filed trails, the review report's findings, the build and verify handoff lines, the ship report, the owner's end-of-turn summaries), it reads for a COLD READER: someone who never opened the issue and does not know what its codename means (owner ruling, 2026-09-09, after a digest whose lines meant nothing with the issue closed).

- One item is exactly ONE bullet, and the issue link LEADS it when one applies (owner ruling, 2026-09-09, #220: the triage and checkpoint trails print this one shape). Sub-bullets under it are fine when they make it easier to read; the item stays one bullet.
...
- Right: "[#158](url): `npm run tower` used to dump omega's whole startup wall or hide everything. Now it prints a starting line, hides the boot chatter, and shows the web server's live request logs. Check by running it." Wrong: "#158 (the tower filter work) is parked at status:qa waiting on your check."

This section is the rule's ONE home. The skills and the owner's personal output style point here and carry no wording of their own; the `manager/profile` hook injects the one-sentence form every prompt, so the shape is in context without a second copy (owner ruling, 2026-09-09: shared logic and prose live in workkit, the personal layer references them).
...
- **Every FULL drain includes the HQ pass** (#100; a scoped run reads only its named issues): after the local sources, triage lists the home repo's open `status:inbox` issues and routes them with the same table, including proposing graduation (below, § The global layer) when a cluster of captures points at one project. No `site.repo`, or HQ unreachable: the pass names the skip; it is never silent. There is no separate HQ command and no HQ session: this pass is how the nursery drains, and it needs only the network, never the clone. HQ's backlog has no session-start announcer either (the `docs:state-check` hook speaks for the cwd repo alone); the tower Board and this pass are its two surfaces.
