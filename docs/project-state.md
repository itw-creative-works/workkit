...
| Surface | Job |
|---|---|
| `settings.json` | HAND-EDITED, and the site options only: `site.repo` (the home repo, below), `site.publish` (the all-or-nothing publish switch) and `site.url` (the custom domain) — an absent key reads as off and as no URL |
