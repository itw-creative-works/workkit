...
**The statuses read as a PIPELINE** — intent to implemented: `status:inbox` → `status:specced` → build → verify → ship → closed against the CHANGELOG entry, with `blocked` and `parked` as side pockets — an answered `blocked` issue rejoins at `status:specced`, a revived `parked` one re-enters triage. Status labels name WAIT states; the working stages have no label of their own — the issue stays `status:specced` while the work runs, and the assignee marks it in flight.
...
Three surfaces, all converging on issues. The only other store is the user-level inbox below, and it converges the same way.
