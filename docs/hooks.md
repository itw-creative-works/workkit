...
- The release commit asks npm for every package with publish intent, each at its own `package.json` version, and asks `github-release` once for the repo at the subject's version. The publish asks npm only: the GitHub release legitimately precedes it in the ship pipeline. A publish naming its workspaces (`--workspace=<x>`, `--workspace <x>`, `-w <x>`, `-w=<x>`) asks about those members alone, matched by package name or by path from the root, so a family published one member at a time is not bounced by the member published a moment ago. Every publishing clause of a chain counts, and one naming no workspace checks the whole set; a name that matches no member, a quoted name and an empty one stand down out loud, and `--workspaces` keeps the whole set.
- The project is the package.json at the git toplevel for a commit and at the cwd for a publish, plus every `workspaces` member (a literal directory, and `dir/*`; any other glob shape is named on stderr and skipped). Publish intent is the publish plan's rule, and [`workflow/publish-plan.js`](../workflow/publish-plan.js) is its home: the hook asks npm about exactly the packages the plan would not skip, plus a workspaces root that carries the intent itself, since a plain `npm publish` at that root publishes it.
