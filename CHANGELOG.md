...
### Added
- [#251](../../issues/251) - A new `safety/release-taken` hook bounces the release commit and the `npm publish` whose version a provider already has: npm for every package with publish intent, workspace members included, and the repo's GitHub release tag. Providers are sibling scripts under the hook, one contract each.

### Changed
- [#267](../../issues/267) - The ship skill's Step 5 says publishing is an allowed action once the safety checks pass and the owner said yes, and names the auto-mode allow rule a repo carries in `.claude/settings.json` under `permissions.allow`: `Bash(npm publish --workspace=*)` for a workspaces monorepo, `Bash(npm publish *)` otherwise.
- [#276](../../issues/276) - The interview asks coupled questions as one: before a round is printed, two questions where one answer settles or reshapes the other merge, the coupled part becoming an option, a sub-bullet or a line inside the question. The test: would the owner's answer to this question answer that one too?
- [#280](../../issues/280) - CI lints the CHANGELOG through the kit's reusable workflow, `.github/workflows/changelog.yml`, one `uses:` line in each repo's `checks.yml`, so no repo carries a linter copy. The heal removes the vendored `.github/changelog-lint.cjs` and rewrites a job still running it; the commit gate counts both as bookkeeping.
- [#277](../../issues/277) - The roster is per machine: the spec no longer says to track `~/.workkit/.repos.json` in dotfiles. Its keys are repo roots in git's spelling, which differ between homes, so a shared copy lost one machine's entries to the other's prune. A fresh machine is asked once per repo.
- [#263](../../issues/263) - The participation predicates have one home, `workflow/participation.sh` beside the platform seam: is this a git repo root (a worktree counts), and is the committed settings file a deliberate no. Every engine, job and hook site calls them instead of its own spelling.
- [#265](../../issues/265) - A test never reaches the real gh from any platform: the seam's one env builder hands a child a scratch home as `HOME`, `USERPROFILE` and `GH_CONFIG_DIR`, and every PATH stub comes from one writer. A Node child on Windows cannot start a shell stub; such cases skip by name.
- [#256](../../issues/256) - Every jq read in the engine, the jobs and the hooks goes through the one CRLF-safe wrapper (`wk_jq`, or `hook_jq` pointing at it), 118 sites in 33 files, so a piped or multi-line read on Windows carries no carriage return. The cloud runner is seeded `platform.sh` beside `lib.sh`.
- [#249](../../issues/249) - Rule files state rules, never their history: every tracked markdown file drops its dated rulings, quoted owner words and issue tags, and the sentence around each is reworded to read as one plain rule. Twin of the dotfiles pass.
- [#248](../../issues/248) - The CHANGELOG entry separator is a spaced hyphen (`- [#4](../../issues/4) - text`), never an em dash: the linter demands it in both forms, the docs and skills show it, and the heal converts a repo's changelog once on its next session.
- [#245](../../issues/245) - The kit runs on macOS, Windows (Git Bash) and Linux: one platform seam and one digest helper in `hooks/_lib.sh`, every marker and cache key named through them, the review and triage markers written by a script, and the engine's address made a real symlink on Windows.
- [#247](../../issues/247) - The Node test suites run on Windows under Git Bash: one seam, `tests/lib/platform.js`, copied name for name from the dotfiles harness, spawns the shell and spells PATH per platform for every hook and script suite, and a case a platform cannot answer skips by name.
- [#255](../../issues/255) - The jobs suites and the tower start suite run on Windows under Git Bash through the same seam as every other suite, and a case a platform cannot answer skips by name; six tower cases that passed vacuously there now skip instead.
- [#246](../../issues/246) - The cold-reader line opens with a bold lead: the item's number, the issue link, then five words, so the owner can reply by number and see what each number is about. The manager line and the checkpoint and triage Filed trails print the same shape.

### Fixed
- [#260](../../issues/260) - A remote spelled as a native Windows path yields its slug: git stores a remote as given, so the backslash form is what it hands the engine there. The shell reader and the tower's Node reader take one character class for both separators.
- [#259](../../issues/259) - Sessions opening at once in several repos all end registered: the heal's machine-level engine link is made idempotently and judged by its result, so a losing `ln` no longer ends a heal before it reaches the roster. The roster lock itself was proven to hold.
- [#258](../../issues/258) - The standards suite runs on Windows: its gh stub named a fixture after a label, and the colon in `agent:working` opens an NTFS data stream that bash cannot see, so six sweep cases failed there. The fixtures are numbered now; the engine needed no change.
- [#266](../../issues/266) - `workkit tower` takes a busy port back on Windows too: the pids on a port and the way one is ended are the platform seam's (`lsof` and `kill` where they ship, `netstat -ano` and `env kill -f -W` on Windows), so the polite-then-forced takeover is one takeover everywhere.
- [#262](../../issues/262) - A `.workkit/settings.json` outside a git repo is nobody's opt-in: the session hook says nothing there and the manager config takes only the machine layer, so a session opened in a temp dir under the Windows profile no longer reads the machine's own file as a repo's.
- [#264](../../issues/264) - A jq read with a fallback takes the fallback only when jq answered nothing: `wk_jq_default` in the platform seam, `hook_jq_default` pointing at it, thirteen sites. A settings or ladder file with a severed tail no longer reads as a doubled answer that matches no branch.
- [#270](../../issues/270) - The test helper that records a stub's argv spells its log path for the shell itself, so a suite hands it the native path and its stubs record under Git Bash too.
- [#272](../../issues/272) - The origin slug has one home per language, `workflow/slug.sh` and `workflow/slug.js`, twins beside each other: the release-taken hook, the canonical-checkout gate, the tower reader and the CHANGELOG link filler all read it there instead of parsing the URL themselves.
- [#274](../../issues/274) - The standards suite keys its decline fixture the way the roster does, in git's spelling, so the case reads the entry the engine kept on Windows instead of missing it under a native key.
- [#275](../../issues/275) - The copied-engine-link case compares the address the shell printed, so the warning that names it passes on Windows as it does elsewhere.
- [#278](../../issues/278) - Every roster fixture in the tests keys its entry in git's spelling through the platform seam, and the tower suites expect that spelling back, so their cases pass on Windows instead of comparing a native path against the roster's key.
- [#271](../../issues/271) - The home suite's poll cases reveal the categories on the gh stub's read count instead of a one-second timer, so the Enter-ahead-of-the-interval case answers the same on Windows as on macOS.
- [#273](../../issues/273) - The standards suite's concurrent-heals case hands its shell the POSIX spelling of the engine files and reads the slug writer's exit code, so a writer that dies at load is loud instead of a null on Windows.
- [#269](../../issues/269) - The tower lists a Windows session: the sessions read takes the keep-awake marker Windows writes (`win.<session>`, alive by its heartbeat, naming its own transcript) beside the macOS one, and the marker directory resolves through the machine's own temp folder instead of a literal `/tmp`.
- [#268](../../issues/268) - The tower derives a transcript path the way Claude Code names its project folders, every character outside letters and digits to a hyphen, so a cwd with an underscore, a space, or a Windows drive finds its transcript.
- [#257](../../issues/257) - `workflow/standards.sh` sources `lib.sh` outright and edits a settings file through its `wk_json_edit`; the twin `edit_settings_json` and the logger fallback it kept for a checkout without `lib.sh` are gone, since no deployment ever ran the heal alone.
- [#261](../../issues/261) - The tower places a session in its repo on Windows: the sessions read publishes the cwd in git's spelling, the roster key's, and the telemetry row reads the transcript path the sessions read named instead of deriving a second one.
- [#254](../../issues/254) - `wk note` outside a project files the issue on the home repo on Windows too: a `.workkit/settings.json` is a repo's opt-in only at a git repo root, so the walk no longer takes the user profile for a participating repo. A severed title is repaired once, never doubled.
- [#253](../../issues/253) - The tower's board keeps its own clone on Windows: a roster key is git's spelling of a repo root at every layer, folded once by `gitPath` in the tower reader and by `wk_git_path` in the engine, so a `C:\...` path and a `C:/...` key are one repo, not two.
- [#252](../../issues/252) - The heal on Windows reads and writes through one CRLF-safe jq, `wk_jq` in `workflow/platform.sh`, the hooks pointing at it: no phantom "missing label" warning on every run, no carriage return in a label create or in a written settings file.
- [#250](../../issues/250) - The brief's summaries board is read once: the daily and the Monday weekly summary are both picked off one Discussions read, so a Monday costs no second round trip and the two `/api/brief` round-trip count tests hold every day of the week instead of failing on Mondays.
