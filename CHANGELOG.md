...
- [#297](../../issues/297) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The tower app suite (5004 lines in one file) is the folder `tests/tower/app/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#298](../../issues/298) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The standards heal suite (3018 lines in one file) is the folder `tests/scripts/workflow-standards/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#299](../../issues/299) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The workkit command suite (1965 lines in one file) is the folder `tests/scripts/workkit-cli/`, one suite per subcommand under the 500-line bar, its shared prologue in `helpers.js`.
- [#300](../../issues/300) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The home lifecycle suite (1595 lines in one file) is the folder `tests/scripts/home/`, one suite per stage under the 500-line bar, its shared prologue in `helpers.js`.
- [#301](../../issues/301) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The commit gate suite (1561 lines in one file) is the folder `tests/hooks/commit-gate/`, one suite per check under the 500-line bar, its shared prologue in `helpers.js`.
- [#302](../../issues/302) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The tower API server suite (1448 lines in one file) is the folder `tests/tower/server/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#303](../../issues/303) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The standards heal (1439 lines in one script) keeps its prologue and main flow in `workflow/standards.sh` and sources its heals from `workflow/standards/`, one file per concern under the 500-line bar; the ship's setup clause names such a folder once.
- [#304](../../issues/304) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The workkit command (1389 lines in one script) keeps its prologue, map and dispatch in `workflow/workkit.sh` and sources its commands from `workflow/workkit/`, one file per concern under the 500-line bar.
- [#305](../../issues/305) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The published site's data layer (1385 lines in one module) keeps its one door in `libs/tower/github.js` and its eight sections in `libs/tower/github/`, one module per concern under the 500-line bar; every name it exported before, it exports after.
- [#306](../../issues/306) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The home repo's lifecycle (1156 lines in one library) keeps its constants and the cloud runner list in `workflow/home.sh` and sources its stages from `workflow/home/`, one file per stage under the 500-line bar; the runner list carries the pieces so the cloud copy sources them too.
- [#307](../../issues/307) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The local morning suite (986 lines in one file) is the folder `tests/jobs/morning-local/`, one suite per step under the 500-line bar, its shared prologue in `helpers.js`.
- [#308](../../issues/308) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The publish suite (925 lines in one file) is the folder `tests/scripts/publish/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#309](../../issues/309) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The tower sweep suite (846 lines in one file) is the folder `tests/tower/board/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#310](../../issues/310) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The tower start suite (834 lines in one file, one group) is the folder `tests/tower/start/`, three suites by topic under the 500-line bar, its shared prologue in `helpers.js`.
- [#311](../../issues/311) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The JSON API (818 lines in one file) keeps its router and wiring in `tower/api/server.js` and its plumbing, validators, feeds and writes in `tower/api/server/`, one module per concern under the 500-line bar; the endpoint's rules and messages are unchanged, and the app's pins read the whole set.
- [#312](../../issues/312) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The morning (776 lines in one script) keeps its prologue and flow in `jobs/morning.sh` and sources its five steps from `jobs/morning/`, one file per step under the 500-line bar; the cloud runner list carries the pieces and the home suite proves it names every one.
- [#313](../../issues/313) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The tower's dialogs (718 lines in one module) keep their one door in `libs/tower/modal.js` and their three dialogs in `libs/tower/modal/`, one module each under the 500-line bar; every name the door exported before, it exports after.
- [#314](../../issues/314) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The telemetry suite (717 lines in one file) is the folder `tests/tower/telemetry/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#315](../../issues/315) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The publish-and-sync suite (704 lines in one file) is the folder `tests/scripts/publish-sync/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#316](../../issues/316) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The cloud morning suite (681 lines in one file) is the folder `tests/jobs/morning-cloud/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#317](../../issues/317) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The crew's spend reader (666 lines in one module) keeps its payload assembly in `tower/api/lib/telemetry.js` and its pricing, bounded read and subagent attribution in `tower/api/lib/telemetry/`, one module per concern under the 500-line bar; the API's answer is unchanged.
- [#318](../../issues/318) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The hooks' shared library (638 lines in one file) keeps its seam and engine sources in `hooks/_lib.sh` and sources its helper groups from `hooks/lib/`, one file per group under the 500-line bar; every hook sources `_lib.sh` as before.
- [#319](../../issues/319) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The tower's shared vocabulary (622 lines in one module) keeps its one door in `libs/tower/format.js` and its five sections in `libs/tower/format/`, one module each under the 500-line bar; every name the door exported before, it exports after.
- [#320](../../issues/320) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The capture guard suite (604 lines in one file) is the folder `tests/hooks/capture-guard/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#321](../../issues/321) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The CHANGELOG backfill suite (601 lines in one file) is the folder `tests/scripts/changelog-links/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#322](../../issues/322) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The page runtime (570 lines in one module) keeps `startPage` and the repo selection in `libs/tower/page.js` and its sidebar selector with the search box in `libs/tower/page/selector.js`, both under the 500-line bar; the runtime's exports are unchanged.
- [#323](../../issues/323) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The publish (566 lines in one flow) keeps its prologue and the order of its steps in `workflow/publish.sh` and sources the steps as functions from `workflow/publish/`, three files under the 500-line bar; every line it prints and every exit code are unchanged.
- [#324](../../issues/324) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The brief payload suite (564 lines in one file) is the folder `tests/jobs/brief-payload/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#325](../../issues/325) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The standards hook suite (561 lines in one file) is the folder `tests/hooks/workflow-standards/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#326](../../issues/326) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The release-taken suite (548 lines in one file) is the folder `tests/hooks/release-taken/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#327](../../issues/327) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The engine's shared library (538 lines in one file) keeps its addresses and every top-level line in `workflow/lib.sh` and sources its voice, its flows and its state helpers from `workflow/lib/`, three files under the 500-line bar; the runner list carries the pieces.
- [#329](../../issues/329) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The commit gate (527 lines in one hook) keeps its parse and classification in `hooks/safety/commit-gate/run.sh` and sources its six checks as functions from `hooks/safety/commit-gate/checks/`, two files under the 500-line bar; every bounce, stand-down and exit code is unchanged.

### Fixed

- [#294](../../issues/294) [`557cdda`](../../commit/557cdda) Thanks [@ianwieds]! - The home copy's exclude list carries the `.env.*` environment overlays the app's `.gitignore` names, so neither the seed nor the sync copies a secret overlay into the published clone.
...
- [#291](../../issues/291) [`59d5d9e`](../../commit/59d5d9e) Thanks [@ianwieds]! - The Board's two groups of lanes lose their card faces: one vertical hairline in the gutter now sets the pocket (blocked and backlog) apart from the pipeline, and the strip no longer carries the padding the faces needed.

### Fixed

- [#292](../../issues/292) [`59d5d9e`](../../commit/59d5d9e) Thanks [@ianwieds]! - The dashboard reads its environment off the build snapshot omega bakes into every page (`OMEGA_BUILD_JSON`), so a dev page under `npm run tower` is live again instead of showing the not-connected card; the global it read before is one the framework stopped writing.

## [0.59.0] - 2026-09-25

### Changed
- [#285](../../issues/285) [`6dde440`](../../commit/6dde440) Thanks [@ianwieds]! - Every skill file is rewritten as bullets under one bar, a body of at most 120 non-blank lines with no line over 400 bytes, pinned by a test.

### Added
- [#289](../../issues/289) [`6dde440`](../../commit/6dde440) Thanks [@ianwieds]! - The Board's column headers wear the status glyph its chips wear, drawn from the one glyph table.
- [#290](../../issues/290) [`6dde440`](../../commit/6dde440) Thanks [@ianwieds]! - Three engine scripts take the ship's mechanical steps: `release.js` makes the release commit's edits, `ship-items.sh` reads the qa and complete items with their proof, and `ci-watch.sh` waits on a sha's CI run and answers by exit code.

### Fixed
- [#286](../../issues/286) [`6dde440`](../../commit/6dde440) Thanks [@ianwieds]! - A ship with a clean tree and `skip` still publishes the tree's version where npm lacks it, and the publish is one command, `publish-plan.js --run`, which asks npm per package and stops at the first failure.
- [#287](../../issues/287) [`6dde440`](../../commit/6dde440) Thanks [@ianwieds]! - Every test suite runs by file: one `selfRun` line ends each, suite discovery lives in `tests/lib/suites.js`, and the runner's own suite fails naming a suite without the line.
- [#288](../../issues/288) [`6dde440`](../../commit/6dde440) Thanks [@ianwieds]! - The tower's nav, topbar and page header icons carry their full Font Awesome classes again, the theme's contract, so they draw instead of rendering blank.
...
### Fixed
- [#281](../../issues/281) [`5d994eb`](../../commit/5d994eb) Thanks [@ianwieds]! - The tower brand config validates against the installed omega again: `targets.web` declares its type and the retired `repo` block is gone (its presence is the switch, and the tower's source is workkit's own repo). The dashboard publish had stopped at the build.

## [0.57.0] - 2026-09-24

### Added
- [#251](../../issues/251) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - A new `safety/release-taken` hook bounces the release commit and the `npm publish` whose version a provider already has: npm for every package with publish intent, workspace members included, and the repo's GitHub release tag. Providers are sibling scripts under the hook, one contract each.

### Changed
- [#267](../../issues/267) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The ship skill's Step 5 says publishing is an allowed action once the safety checks pass and the owner said yes, and names the auto-mode allow rule a repo carries in `.claude/settings.json` under `permissions.allow`: `Bash(npm publish --workspace=*)` for a workspaces monorepo, `Bash(npm publish *)` otherwise.
- [#276](../../issues/276) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The interview asks coupled questions as one: before a round is printed, two questions where one answer settles or reshapes the other merge, the coupled part becoming an option, a sub-bullet or a line inside the question. The test: would the owner's answer to this question answer that one too?
- [#280](../../issues/280) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - CI lints the CHANGELOG through the kit's reusable workflow, `.github/workflows/changelog.yml`, one `uses:` line in each repo's `checks.yml`, so no repo carries a linter copy. The heal removes the vendored `.github/changelog-lint.cjs` and rewrites a job still running it; the commit gate counts both as bookkeeping.
- [#277](../../issues/277) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The roster is per machine: the spec no longer says to track `~/.workkit/.repos.json` in dotfiles. Its keys are repo roots in git's spelling, which differ between homes, so a shared copy lost one machine's entries to the other's prune. A fresh machine is asked once per repo.
- [#263](../../issues/263) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The participation predicates have one home, `workflow/participation.sh` beside the platform seam: is this a git repo root (a worktree counts), and is the committed settings file a deliberate no. Every engine, job and hook site calls them instead of its own spelling.
- [#265](../../issues/265) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - A test never reaches the real gh from any platform: the seam's one env builder hands a child a scratch home as `HOME`, `USERPROFILE` and `GH_CONFIG_DIR`, and every PATH stub comes from one writer. A Node child on Windows cannot start a shell stub; such cases skip by name.
- [#256](../../issues/256) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - Every jq read in the engine, the jobs and the hooks goes through the one CRLF-safe wrapper (`wk_jq`, or `hook_jq` pointing at it), 118 sites in 33 files, so a piped or multi-line read on Windows carries no carriage return. The cloud runner is seeded `platform.sh` beside `lib.sh`.
- [#249](../../issues/249) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - Rule files state rules, never their history: every tracked markdown file drops its dated rulings, quoted owner words and issue tags, and the sentence around each is reworded to read as one plain rule. Twin of the dotfiles pass.
- [#248](../../issues/248) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The CHANGELOG entry separator is a spaced hyphen (`- [#4](../../issues/4) - text`), never an em dash: the linter demands it in both forms, the docs and skills show it, and the heal converts a repo's changelog once on its next session.
- [#245](../../issues/245) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The kit runs on macOS, Windows (Git Bash) and Linux: one platform seam and one digest helper in `hooks/_lib.sh`, every marker and cache key named through them, the review and triage markers written by a script, and the engine's address made a real symlink on Windows.
- [#247](../../issues/247) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The Node test suites run on Windows under Git Bash: one seam, `tests/lib/platform.js`, copied name for name from the dotfiles harness, spawns the shell and spells PATH per platform for every hook and script suite, and a case a platform cannot answer skips by name.
- [#255](../../issues/255) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The jobs suites and the tower start suite run on Windows under Git Bash through the same seam as every other suite, and a case a platform cannot answer skips by name; six tower cases that passed vacuously there now skip instead.
- [#246](../../issues/246) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The cold-reader line opens with a bold lead: the item's number, the issue link, then five words, so the owner can reply by number and see what each number is about. The manager line and the checkpoint and triage Filed trails print the same shape.

### Fixed
- [#260](../../issues/260) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - A remote spelled as a native Windows path yields its slug: git stores a remote as given, so the backslash form is what it hands the engine there. The shell reader and the tower's Node reader take one character class for both separators.
- [#259](../../issues/259) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - Sessions opening at once in several repos all end registered: the heal's machine-level engine link is made idempotently and judged by its result, so a losing `ln` no longer ends a heal before it reaches the roster. The roster lock itself was proven to hold.
- [#258](../../issues/258) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The standards suite runs on Windows: its gh stub named a fixture after a label, and the colon in `agent:working` opens an NTFS data stream that bash cannot see, so six sweep cases failed there. The fixtures are numbered now; the engine needed no change.
- [#266](../../issues/266) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - `workkit tower` takes a busy port back on Windows too: the pids on a port and the way one is ended are the platform seam's (`lsof` and `kill` where they ship, `netstat -ano` and `env kill -f -W` on Windows), so the polite-then-forced takeover is one takeover everywhere.
- [#262](../../issues/262) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - A `.workkit/settings.json` outside a git repo is nobody's opt-in: the session hook says nothing there and the manager config takes only the machine layer, so a session opened in a temp dir under the Windows profile no longer reads the machine's own file as a repo's.
- [#264](../../issues/264) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - A jq read with a fallback takes the fallback only when jq answered nothing: `wk_jq_default` in the platform seam, `hook_jq_default` pointing at it, thirteen sites. A settings or ladder file with a severed tail no longer reads as a doubled answer that matches no branch.
- [#270](../../issues/270) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The test helper that records a stub's argv spells its log path for the shell itself, so a suite hands it the native path and its stubs record under Git Bash too.
- [#272](../../issues/272) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The origin slug has one home per language, `workflow/slug.sh` and `workflow/slug.js`, twins beside each other: the release-taken hook, the canonical-checkout gate, the tower reader and the CHANGELOG link filler all read it there instead of parsing the URL themselves.
- [#274](../../issues/274) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The standards suite keys its decline fixture the way the roster does, in git's spelling, so the case reads the entry the engine kept on Windows instead of missing it under a native key.
- [#275](../../issues/275) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The copied-engine-link case compares the address the shell printed, so the warning that names it passes on Windows as it does elsewhere.
- [#278](../../issues/278) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - Every roster fixture in the tests keys its entry in git's spelling through the platform seam, and the tower suites expect that spelling back, so their cases pass on Windows instead of comparing a native path against the roster's key.
- [#271](../../issues/271) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The home suite's poll cases reveal the categories on the gh stub's read count instead of a one-second timer, so the Enter-ahead-of-the-interval case answers the same on Windows as on macOS.
- [#273](../../issues/273) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The standards suite's concurrent-heals case hands its shell the POSIX spelling of the engine files and reads the slug writer's exit code, so a writer that dies at load is loud instead of a null on Windows.
- [#269](../../issues/269) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The tower lists a Windows session: the sessions read takes the keep-awake marker Windows writes (`win.<session>`, alive by its heartbeat, naming its own transcript) beside the macOS one, and the marker directory resolves through the machine's own temp folder instead of a literal `/tmp`.
- [#268](../../issues/268) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The tower derives a transcript path the way Claude Code names its project folders, every character outside letters and digits to a hyphen, so a cwd with an underscore, a space, or a Windows drive finds its transcript.
- [#257](../../issues/257) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - `workflow/standards.sh` sources `lib.sh` outright and edits a settings file through its `wk_json_edit`; the twin `edit_settings_json` and the logger fallback it kept for a checkout without `lib.sh` are gone, since no deployment ever ran the heal alone.
- [#261](../../issues/261) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The tower places a session in its repo on Windows: the sessions read publishes the cwd in git's spelling, the roster key's, and the telemetry row reads the transcript path the sessions read named instead of deriving a second one.
- [#254](../../issues/254) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - `wk note` outside a project files the issue on the home repo on Windows too: a `.workkit/settings.json` is a repo's opt-in only at a git repo root, so the walk no longer takes the user profile for a participating repo. A severed title is repaired once, never doubled.
- [#253](../../issues/253) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The tower's board keeps its own clone on Windows: a roster key is git's spelling of a repo root at every layer, folded once by `gitPath` in the tower reader and by `wk_git_path` in the engine, so a `C:\...` path and a `C:/...` key are one repo, not two.
- [#252](../../issues/252) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The heal on Windows reads and writes through one CRLF-safe jq, `wk_jq` in `workflow/platform.sh`, the hooks pointing at it: no phantom "missing label" warning on every run, no carriage return in a label create or in a written settings file.
- [#250](../../issues/250) [`e9e9cf4`](../../commit/e9e9cf4) Thanks [@ianwieds]! - The brief's summaries board is read once: the daily and the Monday weekly summary are both picked off one Discussions read, so a Monday costs no second round trip and the two `/api/brief` round-trip count tests hold every day of the week instead of failing on Mondays.

## [0.56.0] - 2026-09-11

### Changed

- [#244](../../issues/244) [`d9fb579`](../../commit/d9fb579) Thanks [@ianwieds]! - `workkit setup` now checks four Discussion categories (Daily, Weekly, Monthly, Brief) and, at a terminal, opens the page that makes the missing ones on Enter and polls every five seconds until they exist (Enter checks now, s skips); the brief's category name has one home.
...
### Added
- [#243](../../issues/243) [`0bfaaa7`](../../commit/0bfaaa7) Thanks [@ianwieds]! - The commit gate now owns the full test suite: the new `safety:suite-guard` hook bounces a bare `npm test` and the repo's test script run directly, from every class alike, so the suite is paid for once at the commit. A narrowed run passes, and `WORKKIT_SUITE=1` is the deliberate full run.

### Changed
- [#241](../../issues/241) [`0bfaaa7`](../../commit/0bfaaa7) Thanks [@ianwieds]! - The tower's Settings page shows one create-token button: the classic-token path is now a link inside the guidance sentence, and the token card and the guidance card share a row at desktop width instead of stacking.

### Fixed
- [#242](../../issues/242) [`0bfaaa7`](../../commit/0bfaaa7) Thanks [@ianwieds]! - The changelog, changelog-links and changelog-guard suites now carry the `require.main` tail every other suite has, so a direct `node <path>` run prints its groups and exits by its result instead of silently reporting nothing.
...
### Added
- [#226](../../issues/226) [`7ac1ee5`](../../commit/7ac1ee5) Thanks [@ianwieds]! - Naming issue numbers in a triage ask now scopes the run to only those issues ("215 211 triage", "accept 215"): no capture-file drain, no home-repo pass, and "accept" waits for your yes unless a real spec is already written. The bare word still drains everything.

### Changed
- [#228](../../issues/228) [`7ac1ee5`](../../commit/7ac1ee5) Thanks [@ianwieds]! - The four tower suites that fake `gh` answers now share one fixture, `tests/lib/gh.js`: the six helpers lived in two full copies and two partial ones, with the error builder under two call forms, and each suite now imports only the names it uses.
- [#222](../../issues/222) [`7ac1ee5`](../../commit/7ac1ee5) Thanks [@ianwieds]! - Review now runs at two cadences: every blind verification asks the drift questions (parity siblings, duplicates by grep, stale docs), every ship runs the full review panel over its whole diff, and a review marker alone no longer licenses a ship.
- [#221](../../issues/221) [`7ac1ee5`](../../commit/7ac1ee5) Thanks [@ianwieds]! - Every restated issue now reads for a cold reader: `docs/project-state.md` § Restating an issue is the one home, the `manager/profile` hook injects it every prompt, every skill and agent that restates issues points there, and the Filed trails print one link-first shape (folds #220).
- [#219](../../issues/219) [`7ac1ee5`](../../commit/7ac1ee5) Thanks [@ianwieds]! - The missing-proof watch moved from the daily heal into the ship: every ship now lists each item at `status:qa` or `status:complete` whose comments carry no `Proof:` line and you call each one, so a spoken pass cannot ship unrecorded.

### Fixed
- [#227](../../issues/227) [`7ac1ee5`](../../commit/7ac1ee5) Thanks [@ianwieds]! - Every skill's frontmatter now parses under a strict YAML loader: the ship description and the migrate argument hint carried a bare colon and a bare bracket that Claude Code tolerated but a stricter loader refuses, and the skills suite now checks every folder for both.
- [#215](../../issues/215) [`7ac1ee5`](../../commit/7ac1ee5) Thanks [@ianwieds]! - The tower's two Discussions reads no longer swallow a `gh` failure: the summaries and the published briefs carry the sweep's own reason beside the null, so the Brief and the Overview name a spent rate limit or a refused token where they said nothing.
- [#211](../../issues/211) [`7ac1ee5`](../../commit/7ac1ee5) Thanks [@ianwieds]! - Triage now resolves which repo an entry belongs to instead of defaulting to the home repo: a finding about a workkit skill, hook, agent or engine file lands on the kit's own repo from any session, another project's through this machine's roster, an unresolvable one stays local.

## [0.51.0] - 2026-09-04

### Added
- [#217](../../issues/217) [`173ebb9`](../../commit/173ebb9) Thanks [@ianwieds]! - The qa stage gains **The proof**: a park names every test layer the change has a surface on in a `Proof:` line of the park comment. The verifier checks the layers, the feature skill cites the rule, and the standards heal names every `status:qa` issue without the line.
- [#212](../../issues/212) [`173ebb9`](../../commit/173ebb9) Thanks [@ianwieds]! - The `workkit:interview` skill now fixes the shape of every question put to the human: a numbered question, its options as nested bullets, the recommended option first and bold with one sentence of why, one bold heading per issue. It is the one home of that shape.
- [#210](../../issues/210) [`173ebb9`](../../commit/173ebb9) Thanks [@ianwieds]! - The `workkit:checkpoint` skill now applies the chat to the board: it enumerates every owner verdict, decision and question first, files or comments each onto its issue, flips the status the owner spoke (a spoken pass on a qa item grants `status:complete`), and reports only what changed.
- [#209](../../issues/209) [`173ebb9`](../../commit/173ebb9) Thanks [@ianwieds]! - The `workkit:review` skill gains a Parity lens: in the full tier a scout names each changed file's siblings and reports where the new code's shape, naming, entry point, logging, or call form differs; the light tier carries one line of it; a finding with no sibling scores 0.

### Changed
- (no issue) [`173ebb9`](../../commit/173ebb9) Thanks [@ianwieds]! - The worker agent now reads the framework guides its brief names before its first edit and records that read through the owning plugin's own command, and the brief-in-a-file convention says a brief must name those guides.

## [0.50.5] - 2026-08-29

### Fixed

- [#216](../../issues/216) [`09a5578`](../../commit/09a5578) Thanks [@ianwieds]! - The tower now reads the rate limit as GitHub actually sends it on GraphQL: an HTTP 200 whose error type is `RATE_LIMIT` with the budget headers beside it, so the Board names the reset time instead of the bare message.
