...
- [#297](../../issues/297) - The tower app suite (5004 lines in one file) is the folder `tests/tower/app/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#298](../../issues/298) - The standards heal suite (3018 lines in one file) is the folder `tests/scripts/workflow-standards/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#299](../../issues/299) - The workkit command suite (1965 lines in one file) is the folder `tests/scripts/workkit-cli/`, one suite per subcommand under the 500-line bar, its shared prologue in `helpers.js`.
- [#300](../../issues/300) - The home lifecycle suite (1595 lines in one file) is the folder `tests/scripts/home/`, one suite per stage under the 500-line bar, its shared prologue in `helpers.js`.
- [#301](../../issues/301) - The commit gate suite (1561 lines in one file) is the folder `tests/hooks/commit-gate/`, one suite per check under the 500-line bar, its shared prologue in `helpers.js`.
- [#302](../../issues/302) - The tower API server suite (1448 lines in one file) is the folder `tests/tower/server/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#303](../../issues/303) - The standards heal (1439 lines in one script) keeps its prologue and main flow in `workflow/standards.sh` and sources its heals from `workflow/standards/`, one file per concern under the 500-line bar; the ship's setup clause names such a folder once.
- [#304](../../issues/304) - The workkit command (1389 lines in one script) keeps its prologue, map and dispatch in `workflow/workkit.sh` and sources its commands from `workflow/workkit/`, one file per concern under the 500-line bar.
- [#305](../../issues/305) - The published site's data layer (1385 lines in one module) keeps its one door in `libs/tower/github.js` and its eight sections in `libs/tower/github/`, one module per concern under the 500-line bar; every name it exported before, it exports after.
- [#306](../../issues/306) - The home repo's lifecycle (1156 lines in one library) keeps its constants and the cloud runner list in `workflow/home.sh` and sources its stages from `workflow/home/`, one file per stage under the 500-line bar; the runner list carries the pieces so the cloud copy sources them too.
- [#307](../../issues/307) - The local morning suite (986 lines in one file) is the folder `tests/jobs/morning-local/`, one suite per step under the 500-line bar, its shared prologue in `helpers.js`.
- [#308](../../issues/308) - The publish suite (925 lines in one file) is the folder `tests/scripts/publish/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#309](../../issues/309) - The tower sweep suite (846 lines in one file) is the folder `tests/tower/board/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#310](../../issues/310) - The tower start suite (834 lines in one file, one group) is the folder `tests/tower/start/`, three suites by topic under the 500-line bar, its shared prologue in `helpers.js`.
- [#311](../../issues/311) - The JSON API (818 lines in one file) keeps its router and wiring in `tower/api/server.js` and its plumbing, validators, feeds and writes in `tower/api/server/`, one module per concern under the 500-line bar; the endpoint's rules and messages are unchanged, and the app's pins read the whole set.
- [#312](../../issues/312) - The morning (776 lines in one script) keeps its prologue and flow in `jobs/morning.sh` and sources its five steps from `jobs/morning/`, one file per step under the 500-line bar; the cloud runner list carries the pieces and the home suite proves it names every one.
- [#313](../../issues/313) - The tower's dialogs (718 lines in one module) keep their one door in `libs/tower/modal.js` and their three dialogs in `libs/tower/modal/`, one module each under the 500-line bar; every name the door exported before, it exports after.
- [#314](../../issues/314) - The telemetry suite (717 lines in one file) is the folder `tests/tower/telemetry/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#315](../../issues/315) - The publish-and-sync suite (704 lines in one file) is the folder `tests/scripts/publish-sync/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#316](../../issues/316) - The cloud morning suite (681 lines in one file) is the folder `tests/jobs/morning-cloud/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#317](../../issues/317) - The crew's spend reader (666 lines in one module) keeps its payload assembly in `tower/api/lib/telemetry.js` and its pricing, bounded read and subagent attribution in `tower/api/lib/telemetry/`, one module per concern under the 500-line bar; the API's answer is unchanged.
- [#318](../../issues/318) - The hooks' shared library (638 lines in one file) keeps its seam and engine sources in `hooks/_lib.sh` and sources its helper groups from `hooks/lib/`, one file per group under the 500-line bar; every hook sources `_lib.sh` as before.
- [#319](../../issues/319) - The tower's shared vocabulary (622 lines in one module) keeps its one door in `libs/tower/format.js` and its five sections in `libs/tower/format/`, one module each under the 500-line bar; every name the door exported before, it exports after.
- [#320](../../issues/320) - The capture guard suite (604 lines in one file) is the folder `tests/hooks/capture-guard/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#321](../../issues/321) - The CHANGELOG backfill suite (601 lines in one file) is the folder `tests/scripts/changelog-links/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#322](../../issues/322) - The page runtime (570 lines in one module) keeps `startPage` and the repo selection in `libs/tower/page.js` and its sidebar selector with the search box in `libs/tower/page/selector.js`, both under the 500-line bar; the runtime's exports are unchanged.
- [#323](../../issues/323) - The publish (566 lines in one flow) keeps its prologue and the order of its steps in `workflow/publish.sh` and sources the steps as functions from `workflow/publish/`, three files under the 500-line bar; every line it prints and every exit code are unchanged.
- [#324](../../issues/324) - The brief payload suite (564 lines in one file) is the folder `tests/jobs/brief-payload/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#325](../../issues/325) - The standards hook suite (561 lines in one file) is the folder `tests/hooks/workflow-standards/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#326](../../issues/326) - The release-taken suite (548 lines in one file) is the folder `tests/hooks/release-taken/`, one suite per concern under the 500-line bar, its shared prologue in `helpers.js`.
- [#327](../../issues/327) - The engine's shared library (538 lines in one file) keeps its addresses and every top-level line in `workflow/lib.sh` and sources its voice, its flows and its state helpers from `workflow/lib/`, three files under the 500-line bar; the runner list carries the pieces.
- [#329](../../issues/329) - The commit gate (527 lines in one hook) keeps its parse and classification in `hooks/safety/commit-gate/run.sh` and sources its six checks as functions from `hooks/safety/commit-gate/checks/`, two files under the 500-line bar; every bounce, stand-down and exit code is unchanged.
