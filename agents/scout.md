...
## Dispatch contract
Your task arrives as a brief FILE, not as chat text — read it. Write your full findings to the report path the brief names, and make your final message a completion status (`DONE` / `DONE_WITH_CONCERNS` / `BLOCKED` / `NEEDS_CONTEXT`) plus ONE line of result. Never spawn subagents.
