...
- Fix the CLASS, not the instance (#222): a hand-typed thing you find wrong once (a signal list, a path join, a rule block) gets grepped across every package before the fix lands. Fix the sites your brief covers, and NAME every other site in the report so the class is fixed once instead of one instance at a time.
