...
# The engine's voice, so the tower's own lines read like every other line the
# kit prints (issue #237): the glyph says what happened. The two halves keep
# their own: what the API prints comes from tower/api/lib/log.js, which speaks
# the same contract in Node, and the app's framework lines pass through raw.
