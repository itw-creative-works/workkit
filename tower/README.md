...
Alongside the pages it writes exactly one file: `data/home.json`, which names the home repo and nothing else — safe to serve to anyone, since the site is served FROM that repo and its own URL already says so. The ROSTER — `data/repos.json`, the `owner/name` slugs the site sweeps plus `home`, the repo its summaries are read from (`workflow/site-repos.js`, from the same roster the tower and the brief read) — goes to the home repo's DEFAULT branch instead (issue #110): Pages is public even when the repo serving it is private, and repo names are private when the repos are. The page reads it back through the GitHub API with the viewer's own token, which is why that token needs Contents: Read on the home repo. Neither file carries a timestamp, so an unchanged roster produces no commit on either branch. Whether anything publishes at all is `site.publish`, default off.
