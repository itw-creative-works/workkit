...
rung_rank() {
  printf '%s' "$config" | hook_jq_default '-1' -r --arg r "$1" '.ladder | keys_unsorted | index($r) // empty'
}

# The class's default rung, via its capability tier.
tier=$(printf '%s' "$config" | hook_jq -r --arg c "$class" '.classes[$c] // empty' 2>/dev/null || true)
rung=$(printf '%s' "$config" | hook_jq -r --arg t "$tier" '.tiers[$t] // empty' 2>/dev/null || true)
[ -n "$rung" ] || exit 0
...
model=$(printf '%s' "$config" | hook_jq -r --arg r "$rung" '.ladder[$r] // empty' 2>/dev/null || true)
[ -n "$model" ] || exit 0
...
tool_input=$(printf '%s' "$input" | hook_jq_default '{}' -c '.tool_input // {}')
hook_jq -n --argjson ti "$tool_input" --arg m "$model" '{
  "hookSpecificOutput": {
